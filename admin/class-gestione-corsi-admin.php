...
//			if (isset($_POST["docente"])) update_post_meta( $post_id, '_docenteCorso', $_POST["docente"]);			
//			delete_post_meta( $post_id, '_docenteCorso');
/**
 *		Memorizzo i Docenti assegnati al corso
 *		Cancello quelli che sono stati eliminati
 *		memorizzo i nuovi
 */
/*			$NewDoc=array();
			if(!is_null($_POST["docente"])){
				foreach($_POST["docente"] as $Docente){
					if($Docente>0){
						$NewDoc[]=$Docente;
					}
				}				
			}
			$NewDoc=array_unique($NewDoc);
			$OldDoc=get_post_meta($post_id, '_docenteCorso');
			$DaEliminare=array_diff($OldDoc,$NewDoc);
			foreach($DaEliminare as $Docente){
				delete_post_meta( $post_id, '_docenteCorso',$Docente);
			}
			$DaAggiungere=array_diff($NewDoc,$OldDoc);
			foreach($DaAggiungere as $Docente){
				add_post_meta( $post_id, '_docenteCorso',$Docente);
			}			
/**
 *		Memorizzo i Tutor assegnati al corso
 *		Cancello quelli che sono stati eliminati
 *		memorizzo i nuovi
 */
/*			$NewTutor=array();
			if(!is_null($_POST["tutor"])){
			...
			$NewTutor=array_unique($NewTutor);
			$OldTutor=get_post_meta($post_id, '_tutorCorso');
//			var_dump($NewTutor);
//			var_dump($OldTutor);
			$DaEliminare=array_diff($OldTutor,$NewTutor);
//			var_dump($DaEliminare);
			foreach($DaEliminare as $Tutor){
				delete_post_meta( $post_id, '_tutorCorso',$Tutor);
			}
			$DaAggiungere=array_diff($NewTutor,$OldTutor);
//			var_dump($DaAggiungere);exit;
			foreach($DaAggiungere as $Tutor){
...
*/
		...
		if(isset($Tutor)){
			foreach($Tutor as $TutorS){
				...
				add_post_meta($post_id, "_tutorCorso", $TutorS[0]);
			}			
		}
					$SortDate=array();
			$SortedLezioni=array();
...
<?php endif; 
	 ...
	 */
	protected function Lista_Corsi($Titolo="",$Id_user=0,$Tipo="_docenteCorso"){
		global $wpdb;
		$Utenti=new Utenti();
		$scope_names = array (
			...
			switch ($scelta){
				case "visform":
					...
					if ( ! wp_verify_nonce($nonce,"VisCorsiFormatore")) {
						...
						$this->Lista_Corsi(" in cui <strong>".ucwords($user_info->last_name)." ".ucwords($user_info->first_name)."</strong> è Formatore",$Id_utente);
				...
				case "vistutor":
					...
					if ( ! wp_verify_nonce($nonce,"VisCorsiTutor")) {
						...
						$user_info = get_userdata($Id_utente);
						$this->Lista_Corsi(" in cui <strong>".ucwords($user_info->last_name)." ".ucwords($user_info->first_name)."</strong> è Tutor",$Id_utente,"_tutorCorso");
